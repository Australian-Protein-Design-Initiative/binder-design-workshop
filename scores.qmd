---
title: "Scores for assessing predicted structures"
format:
  html:
    license: CC BY
...
### `pae_interaction` - inter-chain predicted aligned error
  ...
  - the 'other' `iPAE`
    - [others](https://doi.org/10.1021/acs.jcim.3c00378) [have](http://dx.doi.org/10.1038/s44320-023-00005-6) used the name `iPAE` defined as the _**median PAE of interface residues**_ (where interface residues are those < 3.5 Å from the binding partner). The tools in the workshop don't use this score.
...
### [ipTM](https://www.ebi.ac.uk/training/online/courses/alphafold/inputs-and-outputs/evaluating-alphafolds-predicted-structures-using-confidence-scores/confidence-scores-in-alphafold-multimer/) - interface predicted template modelling (TM) score
  ...
  - a variant, ipSAE, [proposed by Roland Dunbrack](https://doi.org/10.1101/2025.02.10.637595), is calculated in a similar way to ipTM but only considers the most confident pAE pairs in the calculation (ef pAE < 10). This reduces the impact of low confidence 'disordered' regions on the ipTM score.
