...
RFdiffusion is a general tool for generating protein structures via diffusion-based modeling - not only _de novo binder_ design.
...
View some of the designs generated in `output/rfdiffusion` in the Protein Viewer (or locally in ChimeraX/Pymol/etc).

- What do you notice about the sequences of the binders ? **Answer:** [They are all glycine - no binder sequence is predicted at this stage.]{.spoiler}
- Are your binders all in contact with the specific hotspot residues ? **Answer:** [They might be, but aren't guaranteed to be in contact with the hotspots - in particular, if there are large hydrophobic patches nearby, the the model may sometimes use these in preference to the hotspot residues.]{.spoiler}
- Are there any common patterns of secondary structure in these binders generated by RFdiffusion ? **Answer:** [With the default weights, RFDiffusion has a tendency to generate helical binders]{.spoiler}

We don't have any useful scores for the designs at this stage, although we could choose to filter out some designs based on their radius of gyration (`Rg`) if they are too extended, and drop those that are not in the correct binding site. For this exercise, we'll take everything and move on to the next step.
...
Here we randomly sample two sequences for the same backbone structure (`-seqs_per_struct 2`).
You may choose to generate more (five ?) or just one.
