...
> We only have a single GPU on the workshop VMs, however with a few tweaks the above command will work just the same but run on many GPUs in parallel (eg, `-profile slurm`, and `-c nextflow.config` pointing to a nextflow configuation tailored to your HPC cluster and partition access etc)
